...
// temp APIs (S3)
const S3_JSON_URL = "https://plinqer-dev.s3.ap-northeast-2.amazonaws.com/neighbour.json";
...
async function updateUsersLocationByS3() {
    try {
        let usersMap = {}; // id -> {lat, lng, timestamp}

        if (USE_MOCK) {
            // MOCK_USERS => 배열 기반이라 가정
            usersMap = {};
        ...
        // ① 내 위치: 반드시 포함
        // -----------------------------------------
        const meData = usersMap[currentUser.id] || null;
        const me = meData ? {
            id: currentUser.id,
            lat: meData.lat,
            lng: meData.lng,
        ...
        // ② 타인의 위치: 최근 1분 이내만 포함
        // -----------------------------------------
        const others = Object.entries(usersMap)
            .filter(([id]) => id !== currentUser.id) // 나 제외
            .map(([id, u]) => ({
                id,
                lat: u.lat,
                lng: u.lng,
            ...
            .filter(u => now - u.timestamp <= ONE_MINUTE) // 최근 1분 필터
        ...
        console.log("S3 최종 위치 목록:", result);

        // -----------------------------------------
        // 지도 마커 업데이트
        // -----------------------------------------
        const activeIds = new Set(result.map(u => String(u.id)));

        // 내 위치 업데이트
        if (me) updateUserMarker(me);

        // 타인 업데이트
        others.forEach(updateUserMarker);
...
function trackingGps() {
    ...
    if (USE_MOCK) {
        ...
        setInterval(async () => {
            moveAllMockUsers();
            IS_SERVER_ON ? await updateUsersLocation() : await updateUsersLocationByS3();
        }, UPDATE_INTERVAL);
    } else {
        setInterval(async () => {
            IS_SERVER_ON ? await updateUsersLocation() : await updateUsersLocationByS3();
...
async function handleGPS(position) {
    ...
    currentUser.lng = longitude;
    updateUserMarker(currentUser);
    IS_SERVER_ON ? await uploadMyCurrentLocation() : await uploadMyCurrentLocationByS3();
}
